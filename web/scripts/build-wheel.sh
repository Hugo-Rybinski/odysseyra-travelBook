...
#   - the odysseyra_travelbook wheel (fonts bundled), built from the repo, and
...
cd "$out"
tb="$(ls odysseyra_travelbook-*.whl | head -1)"
deps="$(ls *.whl | grep -v '^odysseyra_travelbook-' | sort)"
# odysseyra_travelbook last (nothing depends on install order once deps resolution is off,
# but keep it tidy).
